(u=>{let i={position:"bottom-right",theme:"light",language:"en",customStyles:{}};class t{constructor(e){this.config={...i,...e},this.clientId=this.config.clientId,this.sessionId=null,this.isActive=!1,this.isListening=!1,this.audioContext=null,this.mediaStream=null,this.peerConnection=null,this.dataChannel=null,this.analyser=null,this.volumeInterval=null,this.currentVolume=0,this.messages=[],this.ui=null,this.mode="idle";let t=this.sessionValidationInProgress=!1;if(this.clientId||(console.error("Voice AI SDK: Client ID is required. Please provide a valid clientId in the configuration."),t=!0),this.config.serverUrl||(console.error("Voice AI SDK: Server URL is required. Please provide a valid serverUrl in the configuration."),console.error('Example: serverUrl: "https://voice-ai-sandy.vercel.app"'),t=!0),this.config.serverUrl)try{new URL(this.config.serverUrl)}catch(e){console.error(`Voice AI SDK: Invalid Server URL "${this.config.serverUrl}". Please provide a valid URL.`),t=!0}console.log("Voice AI SDK: Current domain:",u.location.hostname),console.log("Voice AI SDK: Server URL:",this.config.serverUrl),t?console.error("Voice AI SDK: Initialization failed due to configuration errors."):this._init()}async _init(){try{await this._loadSession(),await this._validateClient(),await this._initSession(),this._createUI(),this._registerEventListeners(),"function"==typeof this.config.onReady&&this.config.onReady()}catch(e){console.error("Voice AI SDK: Initialization failed",e),"function"==typeof this.config.onError&&this.config.onError(e)}}async _loadSession(){try{var e,t=localStorage.getItem("voice_ai_session");t?(e=JSON.parse(t)).clientId===this.clientId?(console.log("Voice AI SDK: Found saved session",e.sessionId),await this._validateSession(e.sessionId)?(this.sessionId=e.sessionId,console.log("Voice AI SDK: Session validated successfully",this.sessionId)):(console.log("Voice AI SDK: Saved session is invalid, will create a new one"),localStorage.removeItem("voice_ai_session"),this.sessionId=null)):(console.log("Voice AI SDK: Saved session belongs to different client, will create a new one"),localStorage.removeItem("voice_ai_session")):console.log("Voice AI SDK: No saved session found")}catch(e){console.error("Voice AI SDK: Failed to load session",e),localStorage.removeItem("voice_ai_session"),this.sessionId=null}}async _validateSession(e){if(this.sessionValidationInProgress)return console.log("Voice AI SDK: Session validation already in progress"),!1;this.sessionValidationInProgress=!0;try{console.log("Voice AI SDK: Validating session",e);var t=await fetch(this.config.serverUrl+"/api/v1/sessions?sessionId="+e,{method:"GET"});return this.sessionValidationInProgress=!1,t.ok}catch(e){return console.error("Voice AI SDK: Session validation failed",e),this.sessionValidationInProgress=!1}}_saveSession(){try{localStorage.setItem("voice_ai_session",JSON.stringify({clientId:this.clientId,sessionId:this.sessionId})),console.log("Voice AI SDK: Session saved to localStorage",this.sessionId)}catch(e){console.error("Voice AI SDK: Failed to save session",e)}}async _validateClient(){try{console.log(`Voice AI SDK: Validating client "${this.clientId}" with server at "${this.config.serverUrl}"`),console.log("Voice AI SDK: Current page URL: "+u.location.href);var e=this.config.serverUrl+"/api/v1/auth/validate",t=(console.log("Voice AI SDK: Validation endpoint: "+e),await fetch(e,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({clientId:this.clientId})}));if(!t.ok){var i=t.status;let e;try{e=await t.json()}catch(e){throw new Error(`Server returned ${i} status code. Check if your domain is authorized for this client ID.`)}throw 403===i?(console.error(`Voice AI SDK: Domain "${u.location.hostname}" is not authorized for client "${this.clientId}".`),console.error("Voice AI SDK: Make sure your domain is added to the CLIENT_"+this.clientId+"_DOMAINS environment variable on the server."),new Error(e.error||"Domain not authorized. Status: "+i)):new Error(e.error||"Client validation failed. Status: "+i)}var o=await t.json();if(!o.valid)throw new Error(o.error||"Client validation failed: Server returned invalid status");console.log("Voice AI SDK: Client validated successfully")}catch(e){throw console.error("Voice AI SDK: Client validation failed",e),(e.message.includes("NetworkError")||e.message.includes("Failed to fetch"))&&(console.error("Voice AI SDK: Network error occurred. This might be due to CORS restrictions."),console.error(`Voice AI SDK: Make sure your domain "${u.location.hostname}" is added to the CLIENT_${this.clientId}_DOMAINS environment variable on the server.`),console.error(`Voice AI SDK: Also check that the serverUrl "${this.config.serverUrl}" is correct and accessible.`)),e}}async _initSession(){try{if(this.sessionId)console.log("Voice AI SDK: Using existing session",this.sessionId);else{console.log("Voice AI SDK: Creating new session");var e=await fetch(this.config.serverUrl+"/api/v1/sessions",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({clientId:this.clientId})}),t=await e.json();if(!e.ok)throw new Error(t.error||"Failed to create session");this.sessionId=t.sessionId,console.log("Voice AI SDK: New session created",this.sessionId),this._saveSession()}}catch(e){throw console.error("Voice AI SDK: Session initialization failed",e),e}}_createUI(){var e=document.createElement("div");switch(e.className="voice-ai-container",e.style.position="fixed",this.config.position){case"bottom-right":e.style.bottom="20px",e.style.right="20px";break;case"bottom-left":e.style.bottom="20px",e.style.left="20px";break;case"top-right":e.style.top="20px",e.style.right="20px";break;case"top-left":e.style.top="20px",e.style.left="20px";break;default:e.style.bottom="20px",e.style.right="20px"}var t=document.createElement("div"),i=(t.className="voice-ai-button",t.style.width="64px",t.style.height="64px",t.style.borderRadius="50%",t.style.backgroundColor=this.config.customStyles.buttonColor||"#3a86ff",t.style.display="flex",t.style.alignItems="center",t.style.justifyContent="center",t.style.cursor="pointer",t.style.boxShadow="0 4px 8px rgba(0, 0, 0, 0.2)",t.style.transition="all 0.3s ease",document.createElement("div")),o=(i.className="voice-ai-animation",i.style.width="100%",i.style.height="100%",i.style.display="flex",i.style.alignItems="center",i.style.justifyContent="center",document.createElement("div")),s=(o.className="voice-ai-idle-animation",o.style.width="32px",o.style.height="32px",o.style.display="flex",o.style.alignItems="center",o.style.justifyContent="center",o.style.animation="voice-ai-pulse 2s infinite",document.createElement("img")),s=(s.src=`${this.config.serverUrl||""}/microphone.svg`,s.alt="Microphone",s.style.width="24px",s.style.height="24px",o.appendChild(s),i.appendChild(o),t.appendChild(i),e.appendChild(t),document.createElement("style"));s.textContent=`
        ...
        `,document.head.appendChild(s),document.body.appendChild(e),this.ui={container:e,button:t,animationContainer:i,idleAnimation:o},t.addEventListener("click",()=>{this.toggleSession()})}_registerEventListeners(){u.addEventListener("beforeunload",()=>{this.isActive&&this.stopSession()})}_updateUI(e,s=0){if(this.uiMode=e,this.ui){var t=this.ui.button,i=this.ui.animationContainer,o=this.ui.container.querySelector(".voice-ai-status");this.ui.idleAnimation?.querySelector("img");if(t&&i){for(;i.firstChild;)i.removeChild(i.firstChild);switch(e){case"loading":var n=document.createElement("div");n.className="voice-ai-loading-animation",i.appendChild(n),o&&(o.textContent="Connecting...");break;case"active":var a=document.createElement("div");a.className="voice-ai-volume-container";for(let e=0;e<4;e++){var r=document.createElement("div");r.className="voice-ai-volume-bar",r.style.width="4px",r.style.backgroundColor="#ffffff",r.style.borderRadius="2px",r.style.height="10px",r.style.transition="height 0.3s ease-in-out",a.appendChild(r)}i.appendChild(a),this.ui.volumeBars=Array.from(a.querySelectorAll(".voice-ai-volume-bar")),o&&(o.textContent="Listening...");break;case"thinking":n=document.createElement("div");n.className="voice-ai-thinking-animation",i.appendChild(n),o&&(o.textContent="Thinking...");break;case"responding":n=document.createElement("div");n.className="voice-ai-responding-animation",i.appendChild(n),o&&(o.textContent="Responding...");break;case"error":n=document.createElement("div");n.className="voice-ai-idle-animation",n.style.backgroundColor="#ff3b30",i.appendChild(n),o&&(o.textContent="Error");break;case"volume":if(this.ui.volumeBars&&0<this.ui.volumeBars.length&&i.querySelector(".voice-ai-volume-container"))this.ui.volumeBars.forEach((e,t)=>{var i=.9+.2*Math.random(),i=Math.max(10,Math.min(30,400*s*i)),o=u.getComputedStyle(e),o=parseInt(o.height,10);e.style.height=.7*o+.3*i+"px"});else{var l=document.createElement("div");l.className="voice-ai-volume-container",this.ui.volumeBars=[];for(let e=0;e<4;e++){var c=document.createElement("div"),h=(c.className="voice-ai-volume-bar",c.style.width="4px",c.style.backgroundColor="#ffffff",c.style.borderRadius="2px",c.style.transition="height 0.3s ease-in-out",.9+.2*Math.random()),h=Math.max(10,Math.min(30,400*s*h));c.style.height=h+"px",l.appendChild(c),this.ui.volumeBars.push(c)}i.appendChild(l)}o&&(o.textContent="Listening...");break;default:var n=document.createElement("div"),d=(n.className="voice-ai-idle-animation",document.createElement("img"));d.src=`${this.config.serverUrl||""}/microphone.svg`,d.alt="Microphone",d.style.width="24px",d.style.height="24px",n.appendChild(d),i.appendChild(n),this.ui.idleAnimation=n,o&&(o.textContent="Click to talk")}}}}async _configureSession(){try{console.log("Voice AI SDK: Configuring session with ID:",this.sessionId);var e=await this.peerConnection.createOffer(),t=(await this.peerConnection.setLocalDescription(e),await fetch(this.config.serverUrl+"/api/v1/voice/process",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({clientId:this.clientId,sessionId:this.sessionId,offer:this.peerConnection.localDescription,voice:this.config.voice||"alloy"})})),i=await t.json();if(t.ok)return await this.peerConnection.setRemoteDescription(i.answer),this.instructions=i.instructions,!0;var o=i.error||"Failed to configure session";if(404===t.status&&i.newSessionId)return console.log("Voice AI SDK: Server provided new session ID:",i.newSessionId),this.sessionId=i.newSessionId,this._saveSession(),this._configureSession();if(404===t.status&&o.includes("Session not found"))return console.log("Voice AI SDK: Session not found, creating a new one"),this.sessionId=null,localStorage.removeItem("voice_ai_session"),await this._initSession(),this._configureSession();throw new Error(o)}catch(e){throw console.error("Voice AI SDK: Session configuration failed",e),this._updateUI("error"),e}}_onDataChannelOpen(){console.log("Voice AI SDK: Data channel opened"),setTimeout(()=>{this._configureDataChannel(),this._updateUI("responding")},500)}_configureDataChannel(){this.dataChannel&&"open"===this.dataChannel.readyState?(console.log("Voice AI SDK: Configuring data channel"),(async()=>{try{console.log("Voice AI SDK: Sending session update");var e,t,i={type:"session.update",session:{modalities:["text","audio"],tools:[],turn_detection:{type:"server_vad",threshold:.5,prefix_padding_ms:300,silence_duration_ms:200,create_response:!0},instructions:this.instructions}};this.dataChannel&&"open"===this.dataChannel.readyState&&(this.dataChannel.send(JSON.stringify(i)),await new Promise(e=>setTimeout(e,300)),this.dataChannel)&&"open"===this.dataChannel.readyState&&(console.log("Voice AI SDK: Sending initial message"),e={type:"conversation.item.create",item:{type:"message",role:"user",content:[{type:"input_text",text:"Begin the conversation by introducing yourself as an Improvado representative"}]}},this.dataChannel.send(JSON.stringify(e)),await new Promise(e=>setTimeout(e,500)),this.dataChannel)&&"open"===this.dataChannel.readyState&&(console.log("Voice AI SDK: Requesting response creation"),t={type:"response.create"},this.dataChannel.send(JSON.stringify(t)))}catch(e){console.error("Voice AI SDK: Error sending messages:",e)}})()):console.log("Voice AI SDK: Data channel not open, cannot configure")}_onDataChannelMessage(e){try{var t=JSON.parse(e.data);switch(console.log("Voice AI SDK: Received message",t),t.type){case"thinking":this._updateUI("thinking");break;case"responding":this._updateUI("responding");break;case"response":this.messages.push({role:"assistant",content:t.content}),"function"==typeof this.config.onMessage&&this.config.onMessage({role:"assistant",content:t.content}),this._updateUI("responding");break;case"error":console.error("Voice AI SDK: Error from server",t.error),"function"==typeof this.config.onError&&this.config.onError(new Error(t.error)),this._updateUI("error");break;case"input_audio_buffer.speech_started":this._updateUI("volume",this.currentVolume||.1);break;case"input_audio_buffer.speech_ended":case"output_audio_buffer.started":case"output_audio_buffer.ended":this._updateUI("responding")}}catch(e){console.error("Voice AI SDK: Error parsing message",e)}}_onDataChannelClose(){console.log("Voice AI SDK: Data channel closed"),this.isListening=!1}_onDataChannelError(e){console.error("Voice AI SDK: Data channel error",e)}_onIceCandidate(e){e.candidate&&console.log("Voice AI SDK: New ICE candidate",e.candidate)}_onConnectionStateChange(){if(this.peerConnection)switch(console.log("Voice AI SDK: Connection state changed to",this.peerConnection.connectionState),this.peerConnection.connectionState){case"connected":console.log("Voice AI SDK: WebRTC connection established");break;case"disconnected":case"failed":console.log("Voice AI SDK: WebRTC connection lost"),this._updateUI("error");break;case"closed":console.log("Voice AI SDK: WebRTC connection closed"),this._updateUI("inactive")}}_cleanupWebRTC(){console.log("Voice AI SDK: Cleaning up WebRTC resources"),this.dataChannel&&(this.dataChannel.close(),this.dataChannel=null),this.peerConnection&&(this.peerConnection.close(),this.peerConnection=null),this.mediaStream&&(this.mediaStream.getTracks().forEach(e=>{e.stop()}),this.mediaStream=null),this.isActive=!1,this.isListening=!1,this._updateUI("idle")}async startSession(){if(!this.isActive)try{this._updateUI("loading"),await this._initWebRTC()?(await this._configureSession(),this.isActive=!0,this.isListening=!0,"function"==typeof this.config.onStart&&this.config.onStart()):this._updateUI("error")}catch(e){console.error("Voice AI SDK: Failed to start session",e),this._cleanupWebRTC(),this._updateUI("error"),"function"==typeof this.config.onError&&this.config.onError(e)}}stopSession(){console.log("Voice AI SDK: Stopping session"),this.volumeInterval&&(clearInterval(this.volumeInterval),this.volumeInterval=null),this.volumeTimeout&&(clearTimeout(this.volumeTimeout),this.volumeTimeout=null),this.audioContext&&(this.audioContext.close().catch(console.error),this.audioContext=null,this.analyser=null),this._cleanupWebRTC(),this.isActive=!1,this.isListening=!1,this.currentVolume=0,this._updateUI("idle"),"function"==typeof this.config.onEnd&&this.config.onEnd()}toggleSession(){this.isActive?this.stopSession():(this._updateUI("loading"),this.startSession())}async _initWebRTC(){try{console.log("Voice AI SDK: Initializing WebRTC"),this.peerConnection=new RTCPeerConnection({iceServers:[{urls:"stun:stun.l.google.com:19302"}]}),this.peerConnection.onicecandidate=this._onIceCandidate.bind(this),this.peerConnection.onconnectionstatechange=this._onConnectionStateChange.bind(this),this.stream=await navigator.mediaDevices.getUserMedia({audio:!0}),this._setupAudioVisualization(this.stream),this.stream.getAudioTracks().forEach(e=>{this.peerConnection.addTrack(e,this.stream)}),this.audioElement=document.createElement("audio"),this.audioElement.autoplay=!0,this.peerConnection.ontrack=e=>{console.log("Voice AI SDK: Received audio track"),this.audioElement.srcObject=e.streams[0]},this.dataChannel=this.peerConnection.createDataChannel("response"),this.dataChannel.onopen=this._onDataChannelOpen.bind(this),this.dataChannel.onmessage=this._onDataChannelMessage.bind(this),this.dataChannel.onclose=this._onDataChannelClose.bind(this),this.dataChannel.onerror=this._onDataChannelError.bind(this);var e=await this.peerConnection.createOffer();return await this.peerConnection.setLocalDescription(e),console.log("Voice AI SDK: Created offer"),!0}catch(e){return console.error("Voice AI SDK: Error initializing WebRTC",e),this._updateUI("error"),!1}}_setupAudioVisualization(e){try{var i=new(u.AudioContext||u.webkitAudioContext),o=i.createMediaStreamSource(e);let a=i.createAnalyser();a.fftSize=256,o.connect(a);var s=a.frequencyBinCount;let r=new Uint8Array(s),t=(this.volumeHistory=Array(5).fill(.1),()=>{a.getByteTimeDomainData(r);let t=0;for(let e=0;e<r.length;e++){var i=(r[e]-128)/128;t+=i*i}var e=Math.sqrt(t/r.length);this.volumeHistory.push(e),this.volumeHistory.shift();let o=0,s=0;for(let e=0;e<this.volumeHistory.length;e++){var n=e+1;o+=this.volumeHistory[e]*n,s+=n}return Math.max(.05,o/s)});this.volumeInterval=setInterval(()=>{var e;this.isActive&&this.isListening&&"volume"===this.uiMode&&(e=t(),this._updateUI("volume",e)),this.currentVolume=t()},100),this.audioContext=i,this.analyser=a}catch(e){console.error("Voice AI SDK: Error setting up audio visualization",e)}}_startVolumeDetection(){if(this.analyser){let n=this.analyser.frequencyBinCount,a=new Uint8Array(n);this.volumeHistory||(this.volumeHistory=Array(5).fill(0)),this.volumeInterval=setInterval(()=>{if(this.analyser){this.analyser.getByteFrequencyData(a);let t=0;for(let e=0;e<n;e++)t+=a[e];var e=t/n/255;this.volumeHistory.push(e),this.volumeHistory.shift();let i=0,o=0;for(let e=0;e<this.volumeHistory.length;e++){var s=e+1;i+=this.volumeHistory[e]*s,o+=s}this.currentVolume=i/o,this.isListening&&"volume"===this.uiMode&&this._updateUI("volume",this.currentVolume)}},100)}}_stopVolumeDetection(){this.volumeInterval&&(clearInterval(this.volumeInterval),this.volumeInterval=null)}}u.VoiceAI={init:e=>new t(e)}})(window);